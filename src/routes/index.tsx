...
import { getMockAgentResponse, type AgentResponse, type MemoryState } from "@/services/aiService";

export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "ShopAssist AI · Intelligent customer support" },
      ...
      { property: "og:title", content: "ShopAssist AI · Intelligent customer support" },
...
function ShopAssistApp() {
  ...
  return (
    <div className="app-shell min-h-screen text-foreground">
      ...
      <main>
        {view === "products" && <ProductsView onOpenAssistant={() => setView("assistant")} />}
        {view === "orders" && <OrdersView onOpenAssistant={() => setView("assistant")} />}
        {view === "returns" && <ReturnsView onOpenAssistant={() => setView("assistant")} />}
        {view === "settings" && <SettingsView onOpenAssistant={() => setView("assistant")} />}
      </main>
    </div>
  );
}
      ...
      <main>
        <section className="relative mx-auto grid max-w-7xl items-center gap-10 px-5 py-14 lg:grid-cols-12 lg:px-8 lg:py-24">
          ...
          <div className="relative z-10 lg:col-span-5">
            ...
            <p className="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">ShopAssist AI helps customers find products, track orders, manage returns and discover the right products through one intelligent support agent.</p>
...
function AssistantWorkspace({ onNavigate, mobileNavOpen, onToggleMobileNav }: { onNavigate: (view: View) => void; mobileNavOpen: boolean; onToggleMobileNav: () => void }) {
...
function ChatPanel({ messages, input, setInput, sendMessage, lastResponse, memory, returnSubmitted, setReturnSubmitted }: { messages: ChatMessage[]; input: string; setInput: (value: string) => void; sendMessage: (text: string) => void; lastResponse?: AgentResponse; memory: MemoryState; returnSubmitted: boolean; setReturnSubmitted: (value: boolean) => void }) { const quickActions = [{ label: "Product search", prompt: "Do you have wireless headphones?", icon: Search }, { label: "Track order", prompt: "Where is my order #1024?", icon: PackageSearch }, { label: "Return item", prompt: "I want to return order #1024.", icon: RotateCcw }, { label: "Get recommendation", prompt: "I need headphones under $100.", icon: Sparkles }]; return <div className="mx-auto flex w-full max-w-4xl flex-1 flex-col px-4 py-6 sm:px-8"><div className="mb-5 flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Conversation</p><p className="mt-1 text-sm text-foreground/70">Mock service · ready for your AI API</p></div><div className="flex items-center gap-2 text-xs text-success"><span className="size-1.5 rounded-full bg-success" /> Memory active</div></div><div className="flex-1 space-y-5 overflow-y-auto">{messages.length === 0 ? <WelcomeState quickActions={quickActions} onAction={sendMessage} /> : messages.map((message) => <div key={message.id} className={`message-in flex ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[92%] ${message.role === "user" ? "rounded-2xl rounded-tr-sm bg-glow px-4 py-3 text-background" : "w-full max-w-2xl rounded-2xl rounded-tl-sm border border-line bg-surface px-4 py-4"}`}><p className="text-sm leading-relaxed">{message.text}</p>{message.response && <ResponseContent response={message.response} memory={memory} returnSubmitted={returnSubmitted} setReturnSubmitted={setReturnSubmitted} />}</div></div>)}</div><form onSubmit={(event) => { event.preventDefault(); sendMessage(input); }} className="mt-6 flex items-center gap-2 rounded-xl border border-line bg-surface p-2"><Button type="button" variant="ghost" size="icon" className="text-muted-foreground hover:bg-foreground/10 hover:text-foreground" aria-label="Ask for help"><CircleHelp /></Button><Input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask about a product, order, or return…" className="h-10 border-0 bg-transparent shadow-none focus-visible:ring-0" /><Button type="submit" size="icon" className="bg-glow text-background hover:bg-glow/90" aria-label="Send message"><Send /></Button></form></div>; }
...
function ResponseContent({ response, memory, returnSubmitted, setReturnSubmitted }: { response: AgentResponse; memory: MemoryState; returnSubmitted: boolean; setReturnSubmitted: (value: boolean) => void }) { return <div className="mt-4 space-y-4"><div className="flex items-center gap-2 border-t border-line pt-3 text-xs text-success"><Check className="size-3.5" /> {response.tool} <span className="text-muted-foreground">· Completed</span></div>{response.products && <div className="grid gap-3 sm:grid-cols-3">{response.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div>}{response.orderId && <OrderCard order={orders.find((item) => item.id === response.orderId) ?? orders[0]} />}{response.showReturnForm && <ReturnForm submitted={returnSubmitted} onSubmitted={() => setReturnSubmitted(true)} />}{response.memoryUsed && <div className="flex items-start gap-2 border border-success/20 bg-success/5 p-3 text-xs text-success"><Sparkles className="mt-0.5 size-3.5" /><span><strong>Conversation memory used.</strong> Budget: ${memory.budget} · Category: {memory.category}</span></div>}</div>; }
function AgentActivity({ lastResponse, memory }: { lastResponse?: AgentResponse; memory: MemoryState }) { return <div><div className="flex items-center justify-between"><p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Agent activity</p><span className="flex items-center gap-1.5 text-xs text-success"><span className="size-1.5 rounded-full bg-success" /> Online</span></div><div className="mt-5 space-y-5"><ActivityGroup title="Capabilities" items={["Product search", "Order status", "Return handling", "Recommendations", "Conversation memory"]} /><ActivityGroup title="Recent activity" items={lastResponse ? [lastResponse.tool, "Response generated"] : ["Waiting for a request", "Ready to select a tool"]} /><div><p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Memory</p><div className="mt-3 border border-line bg-background/45 p-3 text-xs text-muted-foreground">{memory.budget ? <><span className="text-success">Active</span><br />Budget: ${memory.budget}<br />Category: {memory.category}</> : <><span className="text-muted-foreground">Active</span><br />Useful preferences will appear here.</>}</div></div></div></div>; }
...
function PageFrame({ title, eyebrow, children, onOpenAssistant }: { title: string; eyebrow: string; children: ReactNode; onOpenAssistant: () => void }) { return <div className="mx-auto max-w-7xl px-5 py-14 lg:px-8 lg:py-20"><div className="flex flex-col justify-between gap-6 border-b border-line pb-8 sm:flex-row sm:items-end"><div><p className="text-xs uppercase tracking-[0.2em] text-glow">{eyebrow}</p><h1 className="mt-3 font-display text-4xl font-semibold tracking-tight">{title}</h1></div><Button onClick={onOpenAssistant} className="rounded-full bg-glow font-semibold text-background hover:bg-glow/90">Ask the assistant <ArrowRight /></Button></div>{children}</div>; }
function ProductsView({ onOpenAssistant }: { onOpenAssistant: () => void }) { const [query, setQuery] = useState(""); const [category, setCategory] = useState<"All" | ProductCategory>("All"); const filtered = products.filter((product) => (category === "All" || product.category === category) && `${product.name} ${product.description}`.toLowerCase().includes(query.toLowerCase())); return <PageFrame title="Products" eyebrow="Product search" onOpenAssistant={onOpenAssistant}><div className="mt-8 flex flex-col gap-3 sm:flex-row"><div className="relative flex-1"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search products…" className="h-11 border-line bg-surface pl-10" /></div><div className="flex gap-2 overflow-x-auto">{["All", "Headphones", "Wearables", "Accessories"].map((item) => <button key={item} onClick={() => setCategory(item as "All" | ProductCategory)} className={`whitespace-nowrap border px-4 text-sm ${category === item ? "border-glow/40 bg-glow/10 text-glow" : "border-line bg-surface text-muted-foreground hover:text-foreground"}`}>{item}</button>)}</div></div><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((product) => <ProductCard key={product.id} product={product} />)}</div>{filtered.length === 0 && <div className="py-20 text-center text-muted-foreground">No products matched that search.</div>}</PageFrame>; }
