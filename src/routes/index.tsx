...
function ResponseContent({ response, memory, returnSubmitted, setReturnSubmitted }: { response: AgentResponse; memory: MemoryState; returnSubmitted: boolean; setReturnSubmitted: (value: boolean) => void }) { const order = response.orderId ? orders.find((item) => item.id === response.orderId) : undefined; return <div className="mt-4 space-y-4"><div className="flex items-center gap-2 border-t border-line pt-3 text-xs text-success"><Check className="size-3.5" /> {response.tool} <span className="text-muted-foreground">· Completed</span></div>{response.products && <div className="grid gap-3 sm:grid-cols-3">{response.products.map((product) => <ProductCard key={product.id} product={product} compact />)}</div>}{order && <OrderCard order={order} />}{response.showReturnForm && <ReturnForm submitted={returnSubmitted} onSubmitted={() => setReturnSubmitted(true)} />}{response.memoryUsed && <div className="flex items-start gap-2 border border-success/20 bg-success/5 p-3 text-xs text-success"><Sparkles className="mt-0.5 size-3.5" /><span><strong>Conversation memory used.</strong> Budget: ${memory.budget} · Category: {memory.category}</span></div>}</div>; }
